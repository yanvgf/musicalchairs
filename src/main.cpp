...
// std::counting_semaphore<NUM_JOGADORES> cadeira_sem(0); // Inicia com n-1 cadeiras, capacidade máxima n
...
//         // Se há apenas um jogador ativo, o jogo termina
//         if (jogadores_ativos.size() == 1) {
//             std::cout << "\n\n+----------------------------------------+" << std::endl;
//             std::cout << "| 🏆 Vencedor: Jogador P" << jogadores_ativos[0] << "! Parabéns! 🏆 |\n";
//             std::cout << "+----------------------------------------+\n" << std::endl;
...
//                 std::cout << "-----------------------------------------------\n" << std::endl;
//                 std::cout << "Iniciando a rodada com " << jogadores_ativos.size() << " jogadores e " << cadeiras << " cadeiras.\n";
...
//             if (!ativo) {
//                 // Se o jogador foi eliminado, sai do loop
//                 break;
//             }
//             // Se o jogador conseguiu uma cadeira, espera a música parar
//             // e tenta novamente
...
//             std::uniform_int_distribution<> distrib(100, 300); // Distribuição uniforme entre 1000 e 5000 ms
...
//             // Espera um tempo fixo pros jogadores terminarem de se sentar
//             std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Ajustável
//         }
//     }
...
// // constexpr int NUM_JOGADORES = 4;
// // // std::counting_semaphore<NUM_JOGADORES> cadeira_sem(0); // Inicia com n-1 cadeiras, capacidade máxima n
// // std::counting_semaphore<NUM_JOGADORES> cadeira_sem(NUM_JOGADORES - 1); // Começa com N-1 cadeiras
// // std::condition_variable music_cv;
// // std::mutex music_mutex;
// // std::atomic<bool> musica_parada{false};
// // std::atomic<bool> jogo_ativo{true};

// // std::mutex jogadores_ativos_mutex;

// // class JogoDasCadeiras {
// // public:
// //     JogoDasCadeiras(int num_jogadores_inicial)
// //         : num_jogadores(num_jogadores_inicial)
// //     {
// //         std::lock_guard<std::mutex> lock(jogadores_ativos_mutex);
// //         jogadores_ativos.resize(num_jogadores_inicial);
// //         std::iota(jogadores_ativos.begin(), jogadores_ativos.end(), 1);
// //         if (num_jogadores_inicial > 1) {
// //              cadeira_sem.release(num_jogadores_inicial - 1);
// //         }
// //     }
...
// //         std::lock_guard<std::mutex> lock_jogadores(jogadores_ativos_mutex); 
// //         std::lock_guard<std::mutex> lock_musica(music_mutex);

// //         int num_ativos = jogadores_ativos.size();
// //         if (num_ativos <= 1) {
// //             if (num_ativos == 1) {
// //                 std::cout << "\n\n+----------------------------------------+" << std::endl;
// //                 std::cout << "| 🏆 Vencedor: Jogador P" << jogadores_ativos[0] << "! Parabéns! 🏆 |\n";
...
// //             musica_parada.store(true);
// //         } else {
// //             int cadeiras_nesta_rodada = num_ativos - 1;
// //             if (cadeiras_nesta_rodada > 0) {
// //                 cadeira_sem.release(cadeiras_nesta_rodada);
// //             }
// //             musica_parada.store(false);

// //             std::cout << "-----------------------------------------------\n" << std::endl;
// //             std::cout << "Iniciando a rodada com " << num_ativos << " jogadores e "
// //                         << cadeiras_nesta_rodada << " cadeiras.\n";
// //             std::cout << "A música está tocando... 🎵";
// //         }
// //         music_cv.notify_all();
// //     }
...
// //     int num_jogadores_ativos() { // Renomeado para clareza
// //         std::lock_guard<std::mutex> lock(jogadores_ativos_mutex);
// //         return jogadores_ativos.size();
// //     }
...
// //                  // Se iniciar_rodada detectou o fim e setou jogo_ativo=false, sai do loop
// //                  if (!jogo_ativo.load()) {
// //                      break;
// //                  }
    
// //                 // Música Tocando
// //                 int tempo_espera_ms = distrib(gen);
// //                 std::this_thread::sleep_for(std::chrono::milliseconds(tempo_espera_ms));
    
// //                 // Parar Música
// //                 if (!jogo_ativo.load()) break; // Verifica de novo
// //                 jogo.parar_musica();
    
// //                 // Tempo pros jogadores terminarem de se sentar
// //                 std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Ajustável
...
// //             // Acorda todas as threads que possam estar esperando
// //             // pra que elas possam verificar jogo_ativo e terminar.
// //             {
// //                 std::lock_guard<std::mutex> lock(music_mutex);
// //                 music_cv.notify_all();
// //             }
...
public:
    ...
    void iniciar_jogo() {
        ...
        while (jogo.num_jogadores_ativos() >= 1) {
            ...
            if (jogo.num_jogadores_ativos() == 1) {
                break;
            }
